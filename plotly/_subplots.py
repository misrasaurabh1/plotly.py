...
def _configure_shared_axes(layout, grid_ref, specs, x_or_y, shared, row_dir):
    ...
    # Inline and optimize what update_axis_matches does, to avoid function call overhead
    def _match_axes_in_line(axis_group, shared_type):
        """
        axis_group: "columns", "rows", or "all".
        Determines looping structure and when to clear tick labels.
        """
        nonlocal layout_key_ind

        if shared_type == "columns" or (x_or_y == "x" and shared is True):
            for c in range(cols):
                first_axis_id = None
                ...
                for r in rows_iter:
                    ...
                    subplot_ref = cell[0]
                    spec = specs[r][c]
                    # Only process for subplots with type xy and span==1
                    if subplot_ref is not None and subplot_ref.subplot_type == "xy" and (
                        (spec["colspan"] if x_or_y == "x" else spec["rowspan"]) == 1
                    ):
                        axis_name = subplot_ref.layout_keys[layout_key_ind]
                        if first_axis_id is None:
                            # Strip "axis" if present
                            if axis_name.endswith("axis"):
                                first_axis_id = axis_name[:-4]
                            else:
                                first_axis_id = axis_name
                        else:
                            axis_to_match = layout[axis_name]
                            axis_to_match.matches = first_axis_id
                            if ok_to_remove_label:
                                axis_to_match.showticklabels = False
        elif shared_type == "rows" or (x_or_y == "y" and shared is True):
            for r in rows_iter:
                first_axis_id = None
                ...
                for c in range(cols):
                    ...
                    if subplot_ref is not None and subplot_ref.subplot_type == "xy" and (
                        ...
                        if first_axis_id is None:
                            ...
                            if ok_to_remove_label:
                                axis_to_match.showticklabels = False
        elif shared_type == "all":
            first_axis_id = None
            for c in range(cols):
                # Precompute ok_to_remove_label for y in this column (always c > 0)
    ...
    if shared == "columns" or (x_or_y == "x" and shared is True):
        _match_axes_in_line("columns", shared)
    elif shared == "rows" or (x_or_y == "y" and shared is True):
        _match_axes_in_line("rows", shared)
    elif shared == "all":
        _match_axes_in_line("all", shared)
