...
class BaseFigure(object):
    ...
    def __init__(
        ...
        # Initialize caches for grid rows/columns and coordinates
        self._subplot_rows_columns_cache = None
        self._subplot_coordinates_cache = None
        self._subplot_coordinates_cache_grid_ref = None  # Avoid coordinate cache if grid changes
...
""",
    ...
    def _get_subplot_rows_columns(self):
        ...
        """
        # Caching: only re-compute if grid has changed
        grid_ref = self._validate_get_grid_ref()
        cache = getattr(self, "_subplot_rows_columns_cache", None)
        cache_key = id(grid_ref)
        # The cache will be a tuple (cache_key, (rows, cols))
        if cache is not None and cache[0] == cache_key:
            return cache[1]
        nrows = len(grid_ref)
        ncols = len(grid_ref[0])
        result = (range(1, nrows + 1), range(1, ncols + 1))
        self._subplot_rows_columns_cache = (cache_key, result)
        return result
    ...
    def _get_subplot_coordinates(self):
        ...
        """
        grid_ref = self._validate_get_grid_ref()
        # Only re-generate coordinates if grid_ref has changed
        if (
            self._subplot_coordinates_cache is not None
            and self._subplot_coordinates_cache_grid_ref is grid_ref
        ):
            return iter(self._subplot_coordinates_cache)
        rows, cols = self._get_subplot_rows_columns()
        # Materialize list only once for reuse (memory trade-off, but usually small)
        coords = list(itertools.product(rows, cols))
        self._subplot_coordinates_cache = coords
        self._subplot_coordinates_cache_grid_ref = grid_ref
        return iter(coords)
